...
import Repository.repository_NV;
...
public class Form_NhanVien extends javax.swing.JPanel {
    ...
    private repository_NV repNV = new repository_NV();
    private DefaultTableModel dtm = new DefaultTableModel();

    public Form_NhanVien() {
        initComponents();
        dtm = (DefaultTableModel) tbl_NhanVien.getModel();
        showDataTable(repNV.getAll());
    }

    private void showDataTable(List<Model_NV> lst) {
        dtm.setRowCount(0);
        for (Model_NV nv : lst) {
            dtm.addRow(new Object[]{
                nv.getTenNV(), nv.getMaNV(), nv.getSdt(), nv.isTrangThai() ? "Đang làm" : "Đã Nghỉ"
            });
        }
    }

    private void showDetail(int index) {
        Model_NV nv = repNV.getAll().get(index);
        txt_TenNV.setText(nv.getTenNV());
        txt_maNV.setText(nv.getMaNV());
        txt_SDT_NV.setText(nv.getSdt());
        txt_TK.setText(nv.getTK());
        txt_MK.setText(nv.getMK());
        rdo_NV_danghi.setSelected(!nv.isTrangThai());
        rdo_NvDangLam.setSelected(nv.isTrangThai());
    }

    private Model_NV getForm() {
        ...
        Boolean trangThai = rdo_NvDangLam.isSelected();

        return new Model_NV(ma, ten, sdt, tk, mk, trangThai);
    }
    ...
    private boolean checkEmpty() {
        ...
        if (txt_SDT_NV.getText().trim().isEmpty()) {
    ...
    private void btn_SuaNVActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_SuaNVActionPerformed
        ...
        String ma = repNV.getAll().get(index).getMaNV();
        repNV.update(getForm(), ma);
        showDataTable(repNV.getAll());
